...
As time passed I got curious about CLI AI tools so some has been used to test their capabilities after i wrote the original scripts by hand. Improvements and bug fixes will be done without AI to ensure there are NO MISTAKES. This will likely be one of the only project I'll be publicly using AI for. Although my motivation to make these scripts are strictly for personal use so if they have bugs or security issues im not too concerned if the AI made any mistakes. Which there was many. So im not entirely convinced AI actually is a time saver rather than a procastinator a majority of the time. From my experience using CLI AI in this project the fact still stands that experience and critical thinking trumps AI. The only time it has been useful is writing repetitive code, which it will still make mistakes. So it needs to be able to reliably test its output to produce anything of value. Which is another time sink... 

Yes it can write usable code thats beyond user's knowledge, but I find that doing this is unwise as it becomes impossible to validate anything its doing which as stated, will result in wasting tokens, energy, time, and money.
